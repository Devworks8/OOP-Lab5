	...
	{ 
		...
		public void SetEmitPhonons(double tEq, double effEnergy, double timeStep)
		{
			foreach (ISurface surface in surfaces)
			{
				if (surface is EmitSurface emitSurface)
				{
					//Check for location Width or Length
					if (emitSurface.Location is SurfaceLocation.left || emitSurface.Location is SurfaceLocation.right)
					{
						emitSurface.SetEmitPhonons(tEq, effEnergy, timeStep, Width);
					}
					if (emitSurface.Location is SurfaceLocation.top || emitSurface.Location is SurfaceLocation.bot)
                    {
		...
		{
			return string.Format("{0,-20} {1,-7} {2,-7}\n {3,-20} {4,-7} {5,-7}\n", "", "Ttl", "Inc", sensor.ToString(), phonons.Count, incomingPhonons.Count);
		}
